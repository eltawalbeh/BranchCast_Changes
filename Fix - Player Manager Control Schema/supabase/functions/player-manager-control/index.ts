...
Deno.serve(async (request) => {
  ...
  const member = await admin.from('organization_members').select('user_id').eq('organization_id', location.data.organization_id).eq('user_id', identity.data.user.id).maybeSingle()
