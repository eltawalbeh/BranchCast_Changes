    ...
    return (
          <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                                  <h2 className="text-lg font-bold text-white">{isAr ? 'فروع الشركة والمناطق الصوتية' : 'Branches & Audio Zones'}</h2>h2>
                                  <p className="text-xs text-zinc-400 mt-0.5">
                                    {isAr ? 'إدار مكبرات الصوت والأجهزة المقترنة في إبد وعمّان' : 'Manage paired sound systems and zones in Irbid & Amman'}
                                  </p>p>
                        </div>div>
                        <button className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-zinc-950 font-semibold text-xs rounded-lg transition-colors shadow-md">
                          {isAr ? '+ إضافة فع جديد' : '+ Add New Branch'}
                        </button>button>
                </div>div>
                  ...
                  {branches.map(b => (
                      <div key={b.id} className="p-5 rounded-xl bg-zinc-950 border border-zinc-800 hover:border-zinc-700 transition-colors">
                                  ...
                                  <div className="mt-4 pt-4 border-t border-zinc-800/80 flex items-center justify-between text-xs text-zinc-400">
                                                ...
                                                <span className="text-emerald-400 font-medium">{isAr ? 'مشغ Web Player v1.0' : 'Web Player v1.0'}</span>span>
                                  </div>div>
                      </div>div>
                    ))}
                </div>div>
