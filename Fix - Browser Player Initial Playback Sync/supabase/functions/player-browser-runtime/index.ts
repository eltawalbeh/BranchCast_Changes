...
Deno.serve(async (request) => {
  ...
  const session = await admin.from('player_browser_sessions').select('id,player_id,expires_at,revoked_at,created_at').eq('token_hash', tokenHash).maybeSingle()
  ...
  if (!result.data) {
    ...
    if (schedule.data && scheduleContentId && schedule.data.id !== player.data.active_schedule_entry_id) {
      ...
      result.data = command.data
    } else if (player.data.active_content_item_id) {
      const lastPlay = await admin.from('player_commands').select('id,created_at').eq('player_id', session.data.player_id).eq('command', 'play').order('created_at', { ascending: false }).limit(1).maybeSingle()
      const needsInitialSync = !lastPlay.data || new Date(lastPlay.data.created_at).getTime() < new Date(session.data.created_at).getTime()
      if (!needsInitialSync) return json({ command: null })
      const command = await admin.from('player_commands').insert({ player_id: session.data.player_id, requested_by: null, command: 'play', payload: { content_item_id: player.data.active_content_item_id, source: 'browser-session-sync' }, status: 'pending' }).select('id,command,payload,created_at').single()
      if (command.error) return json({ error: command.error.message }, 500)
      result.data = command.data
    } else {
