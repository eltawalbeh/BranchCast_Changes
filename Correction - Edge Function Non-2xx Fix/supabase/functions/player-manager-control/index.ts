...
Deno.serve(async (request) => {
  ...
  if (!url || !serviceRole) return json({ error: 'Function configuration is incomplete' }, 500)
  ...
  if (zone.error || !zone.data) return json({ error: zone.error?.message || 'Player zone not found' }, 404)
  const location = await admin.from('locations').select('organization_id').eq('id', zone.data.location_id).single()
  if (location.error || !location.data) return json({ error: location.error?.message || 'Player location not found' }, 404)
  ...
  if (!member.data) return json({ error: 'You do not have access to this player' }, 403)
  ...
  const update = body.action === 'stop'
    ? { pairing_code: code, pairing_expires_at: expiry, last_error: null }
    : { pairing_code: code, pairing_expires_at: expiry, paired_at: null, last_error: null }
